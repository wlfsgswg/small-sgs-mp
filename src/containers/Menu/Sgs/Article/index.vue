          ...
          <div class="top-flex">
            <div class="name-lv-op">
              <div class="name-lv">
                <div class="name">
                  {{
                    item.user && item.user.nick_name.length > 5
                      ? `${item.user && item.user.nick_name.slice(0, 5)}...`
              ...
              <div class="op">
                <span
                  v-if="item.focus === '0'"
                  class="m-r-10 span"
                  @click="handleOp('0', item)"
                  >关注</span
                >
                <span v-if="item.focus === '1'" class="m-r-10 span-not"
                  >已关注</span
                >
                <span
                  v-if="item.black === '0'"
                  class="span"
                  @click="handleOp('1', item)"
                  >拉黑</span
                >
                <span v-if="item.black === '1'" class="m-r-10 span-not"
                  >已拉黑</span
                >
              </div>
            </div>
        ...
        <div class="bottom">
          <div class="view">
            <img
              ...
              alt=""
              class="icon"
            /><span class="likeText">{{ item.readnum }}</span>
            <img
              ...
              alt=""
              class="icon"
            /><span class="likeText">{{ item.replynum }}</span>
            <div style="align-items: center; display: flex">
              <img
                ...
                alt=""
                class="icon"
              /><span class="likeText">{{ item.likenum }}</span>
            </div>
          </div>
          <div class="delete">
            <span class="span" @click="deleteItem(item)">删除</span>
          </div>
        </div>
      </div>
    </div>
              ...
              <template slot="title">
                <div class="wx-account">
                  ...
                  <div class="base-data">
                      ...
                      <div class="account m-r-20">
                        {{
                          opendata.user && opendata.user.nick_name.length > 5
                            ? `${
                                opendata.user &&
                                opendata.user.nick_name.slice(0, 5)
                              }...`
                    ...
                    <div class="icons">
                      <div class="icon">
                        阅读：<span class="icon-num">{{
                          opendata.readnum
                        }}</span>
                      </div>
                      <div class="icon">
                        评论：<span class="icon-num">{{
                          opendata.replynum
                        }}</span>
                      </div>
                      <div class="icon">
                        点赞：<span class="icon-num">{{
                          opendata.likenum
                        }}</span>
                      </div>
                    </div>
                  ...
                  <div class="clearfix wx-account-op">
                    <div class="m-r-20">
                      <el-button
                        type="text"
                        class="right-btn-32"
                        @click="handleCopy(opendata.title)"
                        >复制标题</el-button
                      >
                    </div>
                    <div class="m-r-20">
                      <el-button
                        type="text"
                        class="right-btn-32"
                        @click="handleCopy(opendata.body)"
                        >复制内容</el-button
                      >
                    </div>
                    <div class="select">
                      <el-select
                        ...
                        size="small"
                        prefix="洗稿账号"
                        v-model="opendata.copyids"
                        :style="{ width: '100%' }"
                        placeholder="请选择洗稿账号"
                      >
                        <el-option
                          v-for="item in mpwxaccountList.filter(
                            (it) => it.isself !== '0' && it.iscancel !== '1',
                          )"
                          :value="item.id"
                          :label="item.name"
                          :key="item.id"
                  ...
                  <div
                    class="imgs"
                    v-for="item in opendata.body_image"
                    :key="item"
                  >
...
export default {
  ...
  methods: {
      ...
      this.$API
        ...
        .then((res) => {
          res.data.map((it) => {
            ...
            // 拉黑或关注
            const currentItem = this.accountList.filter(
              (item) => item.id === (it.user && it.user.id),
            );
    ...
    handleGetAccountlist() {
      this.$API.getXhAccount().then((res) => {
    ...
    deleteItem(e) {
      //请求ajax删除
      this.$API.deleteData({ id: e.id, keyname: "id" }).then(() => {
        this.list = this.list.filter((it) => it.id !== e.id);
